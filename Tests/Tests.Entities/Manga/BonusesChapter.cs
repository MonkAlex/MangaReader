...
namespace Tests.Entities.Manga
{
  ...
  public class BonusesChapter : TestClass
  {
    ...
    {
      var manga = await Mangas.CreateFromWeb(new Uri("http://mangachan.me/manga/5335-the-breaker-new-waves.html"));
    ...
    {
      var manga = await Mangas.CreateFromWeb(new Uri("http://readmanga.me/animal_country"));
      await manga.Parser.UpdateContent(manga);
    ...
    public async Task MintmangaBonus()
    {
      using (var context = Repository.GetEntityContext())
      {
        ...
        var manga = await Mangas.CreateFromWeb(uri);
        await manga.Parser.UpdateContent(manga);
        var chapters = manga.Volumes.SelectMany(v => v.Container).OfType<Grouple.GroupleChapter>();
