...
namespace Tests.Entities.Manga
{
  ...
  public class HentaichanStructure : TestClass
  {
    ...
    {
      var manga = await GetManga($"{Constants.HentaichanHost}manga/14212-love-and-devil-glava-25.html").ConfigureAwait(false);
    ...
    {
      var manga = await GetManga($"{Constants.HentaichanHost}manga/15131-chuui-horeru-to-yakui-kara.html").ConfigureAwait(false);
    ...
    {
      var manga = await GetManga($"{Constants.HentaichanHost}manga/23083-ponpharse-tokubetsu-hen-chast-1.html").ConfigureAwait(false);
    ...
    {
      // Спецсимвол \
      await TestNameParsing($"{Constants.HentaichanHost}manga/14504-lets-play-lovegames-shall-we-glava-1.html",
        "Let's Play Lovegames, Shall We?").ConfigureAwait(false);

      // Спецсимвол # и одна глава
      await TestNameParsing($"{Constants.HentaichanHost}manga/15109-exhibitionist-renko-chan.html",
        "#Exhibitionist Renko-chan").ConfigureAwait(false);

      // Символ звездочки *
      await TestNameParsing($"{Constants.HentaichanHost}manga/15131-chuui-horeru-to-yakui-kara.html",
        "*Chuui* Horeru to Yakui kara").ConfigureAwait(false);

      // Символ /
      await TestNameParsing($"{Constants.HentaichanHost}manga/10535-blush-dc.-glava-1.html",
        "/Blush-DC.").ConfigureAwait(false);

      // На всякий случай
      await TestNameParsing($"{Constants.HentaichanHost}manga/23083-ponpharse-tokubetsu-hen-chast-1.html",
        "Ponpharse - Tokubetsu Hen").ConfigureAwait(false);

      // Манга требующая регистрации для просмотра
      await TestNameParsing($"{Constants.HentaichanHost}manga/14212-love-and-devil-glava-25.html",
